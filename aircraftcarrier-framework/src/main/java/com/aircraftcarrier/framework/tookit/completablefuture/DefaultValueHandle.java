...
/**
 * 当发生异常时返回自定义的值
 * eg: completableFuture.handle(new DefaultValueHandle<>("orderService.getOrder", Collections.emptyMap(), params));
 *
 * @author meituan
 */
public class DefaultValueHandle<R> extends AbstractLogAction<R> implements BiFunction<R, Throwable, R> {
    ...
    public static class DefaultValueHandleBuilder<R> {
        ...
        public DefaultValueHandle<R> build() {
            return new DefaultValueHandle<>(this.isNullToDefault, this.defaultValue, this.methodName, this.args);
        }
