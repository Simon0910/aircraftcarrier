 ...
 * the License.
 */
package com.aircraftcarrier.generator.engine;

import com.aircraftcarrier.generator.config.ConstVal;
...
import com.aircraftcarrier.generator.config.builder.ConfigBuilder;
import com.aircraftcarrier.generator.config.po.TableInfo;
import com.aircraftcarrier.generator.custom.FileOutConfig;
import com.aircraftcarrier.generator.util.FileUtils;
import com.aircraftcarrier.generator.util.RuntimeUtils;
...
public abstract class AbstractTemplateEngine {
     ...
     * @param objectMap
     */
    protected void outputCustomFile(@NotNull ConfigBuilder config, @NotNull TableInfo tableInfo, @NotNull ConcurrentHashMap<String, Object> objectMap) {
        Optional.ofNullable(config.getInjectionConfig()).ifPresent(t -> {
            t.beforeOutputFile(tableInfo, objectMap);

            List<FileOutConfig> fileOutConfigs = t.getFileOutConfigs();
            Map<String, String> outputFilePathMap = CollectionUtils.newHashMapWithExpectedSize(fileOutConfigs.size());
            ConcurrentHashMap<String, String> paramsMap = new ConcurrentHashMap<>(CollectionUtils.newHashMapWithExpectedSize(fileOutConfigs.size() * 4));
            for (FileOutConfig fileOutConfig : fileOutConfigs) {
                String outputFilePath = fileOutConfig.outputFile(tableInfo);
                paramsMap.put(fileOutConfig.getKey() + "_p", fileOutConfig.outClassPackage());
            ...
            if (oldParamsMap != null) {
                oldParamsMap.putAll(paramsMap);
            } else {
                objectMap.put(CFG, paramsMap);
            }
            for (FileOutConfig fileOutConfig : fileOutConfigs) {
                outputFile(new File(outputFilePathMap.get(fileOutConfig.getKey())), objectMap, fileOutConfig.getTemplatePath(), getConfigBuilder().getInjectionConfig().isFileOverride());
            }
    ...
    public AbstractTemplateEngine batchOutput() {
        try {
            ...
            tableInfoList.forEach(tableInfo -> {
                ...
                // entity
                outputEntity(tableInfo, newObjectMap);
                // mapper and xml
                outputMapper(tableInfo, newObjectMap);
                // service
                outputService(tableInfo, newObjectMap);
                // controller
                outputController(tableInfo, newObjectMap);
            });
        } catch (Exception e) {
